...
if ! { [[ "$1" == "test" ]] && cd "$gpath/git-db"; }; then
    DB_VERSION_NEW=$(curl --silent --show-error "https://raw.githubusercontent.com/adsbfi/tar1090-db/master/version")
...
if [[ "$1" == "test" ]] || [[ -n "$git_source" ]]; then
    ...
    VERSION_NEW=$(curl --silent --show-error "https://raw.githubusercontent.com/adsbfi/tar1090/globe/version")
    if  [[ "$(cat "$gpath/git/version" 2>/dev/null)" != "$VERSION_NEW" ]]; then
        if ! getGIT "$repo" "globe" "$gpath/git"; then
            echo "Unable to download files, exiting! (Maybe try again?)"
            exit 1
        fi
    fi
